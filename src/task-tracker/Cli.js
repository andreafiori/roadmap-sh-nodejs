...
class Cli {
    ...
    listTasks(status) {
        ...
        tasks.forEach((task) => {
            const inProgressColor = task.inProgress ? this.colors.yellow : this.colors.red;
            const inProgressStatus = task.inProgress ? "In-progress" : "To-do";
            const statusColor = task.completed ? this.colors.green : inProgressColor;
            const statusText = task.completed ? "Done" : inProgressStatus;
            console.log(
    ...
    updateTask(id, newDescription) {
        ...
        if (this.taskManager.updateTask(Number.parseInt(id), newDescription)) {
    ...
    deleteTask(id) {
        ...
        if (this.taskManager.deleteTask(Number.parseInt(id))) {
    ...
    markInProgress(id) {
        if (!this.validateId(id)) return;
        if (this.taskManager.markInProgress(Number.parseInt(id))) {
    ...
    markDone(id) {
        if (!this.validateId(id)) return;
        if (this.taskManager.markDone(Number.parseInt(id))) {
